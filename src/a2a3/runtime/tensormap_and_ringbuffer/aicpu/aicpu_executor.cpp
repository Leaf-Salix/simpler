...
int32_t AicpuExecutor::run(
    ...
    if (!sched_ctx_.is_completed() && thread_idx < sched_thread_num_ &&
        (kernel_thread == nullptr || kernel_thread->execution_index >= 0)) {
...
int32_t AicpuExecutor::complete_kernel_init() {
    const int32_t status = sched_ctx_.post_handshake_init(kernel_invocation_.resident(), kernel_invocation_.inputs().functions);
...
void release_kernel_execution() {
    if (!g_aicpu_executor.kernel_gate_.idle()) return;
...
extern "C" __attribute__((visibility("default"))) int simpler_aicpu_prepare_tmr_context(void *arg) {
    return simpler::tmr::register_kernel_context(g_aicpu_executor, arg);
}
extern "C" __attribute__((visibility("default"))) int simpler_aicpu_register_tmr_kernel_callable(void *arg) {
    return simpler::tmr::register_kernel_callable(g_aicpu_executor, arg);
}
extern "C" __attribute__((visibility("default"))) int simpler_aicpu_release_tmr_kernel_callable(void *arg) {
    return simpler::tmr::release_kernel_callable(g_aicpu_executor, arg);
}
extern "C" __attribute__((visibility("default"))) int simpler_aicpu_release_tmr_context(void *arg) {
    return simpler::tmr::release_kernel_context(g_aicpu_executor, arg);
}
