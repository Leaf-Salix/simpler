...
void SchedulerContext::log_scheduler_stall_snapshot(
    ...
    if (slot_state != nullptr && slot_state->task != nullptr) {
        task_id = static_cast<int64_t>(slot_state->task->task_id.raw);
        ...
        available = count_global_available(shape, core_mask, gated != 0);
    }
    ...
    for (int32_t t = 0; t < thread_count; t++) {
        ...
        const int32_t *core_ids = tracker.core_ids();
        for (int32_t i = 0; i < tracker.core_num(); i++) {
        ...
        for (int32_t i = 0; i < tracker.core_num(); i++) {
            int32_t core_id = core_ids[i];
            CoreExecState &core = core_exec_states_[core_id];
            PTO2TaskSlotState *running = core.running_slot_state;
            PTO2TaskSlotState *pending = core.pending_slot_state;
            int64_t running_id =
                running != nullptr && running->task != nullptr ? static_cast<int64_t>(running->task->task_id.raw) : -1;
            int64_t pending_id =
                pending != nullptr && pending->task != nullptr ? static_cast<int64_t>(pending->task->task_id.raw) : -1;
            int32_t running_fanin = running != nullptr ? running->fanin_refcount.load(std::memory_order_relaxed) : -1;
            int32_t running_fanin_total = running != nullptr ? running->fanin_count : -1;
            int32_t pending_fanin = pending != nullptr ? pending->fanin_refcount.load(std::memory_order_relaxed) : -1;
            int32_t pending_fanin_total = pending != nullptr ? pending->fanin_count : -1;
            int32_t running_ed = running != nullptr && running->payload != nullptr ?
                                     running->payload->early_dispatch_state.load(std::memory_order_relaxed) :
                                     -1;
            int32_t pending_ed = pending != nullptr && pending->payload != nullptr ?
                                     pending->payload->early_dispatch_state.load(std::memory_order_relaxed) :
                                     -1;
