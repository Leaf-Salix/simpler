...
#if SIMPLER_DFX
static bool log_heap_hol_consumers(PTO2SchedulerState *sched, uint8_t head_ring_id, int32_t head_task_id) {
    PTO2SchedulerState::RingSchedState &head_sched = sched->ring_sched_states[head_ring_id];
    int32_t expected = 0;
    ...
    if (last_alive != head_task_id) {
        head_sched.advance_lock.store(0, std::memory_order_release);
        return false;
    }
    ...
    PTO2TaskState head_state = head.task_state.load(std::memory_order_acquire);
    head_sched.advance_lock.store(0, std::memory_order_release);
    ...
    int32_t completed = 0;
    int32_t consumed = 0;
    int32_t logged_pending_consumers = 0;
    constexpr int32_t max_pending_consumer_logs = 8;
    for (int32_t ring_id = 0; ring_id < PTO2_MAX_RING_DEPTH; ring_id++) {
        ...
        for (int32_t task_id = last_alive; task_id < current; task_id++) {
            ...
            PTO2TaskDescriptor *consumer_task = consumer.task;
            PTO2TaskPayload *payload = consumer.payload;
            if (consumer_task == nullptr || payload == nullptr || payload->fanin_spill_pool == nullptr ||
                static_cast<int32_t>(consumer_task->task_id.local()) != task_id) {
                continue;
            }
            ...
            int32_t fanin_refcount = consumer.fanin_refcount.load(std::memory_order_acquire);
            if (state == PTO2_TASK_PENDING) {
                pending++;
                if (fanin_refcount >= consumer.fanin_count) pending_ready++;
            ...
            if (state != PTO2_TASK_PENDING || logged_pending_consumers >= max_pending_consumer_logs) continue;
            logged_pending_consumers++;
            int32_t producer_pending = 0;
            int32_t producer_completed = 0;
            int32_t producer_consumed = 0;
            int64_t first_pending_producer = -1;
            int32_t first_pending_fanin_refcount = -1;
            int32_t first_pending_fanin_count = -1;
            int32_t first_pending_completed_subtasks = -1;
            int32_t first_pending_total_subtasks = -1;
            int32_t first_pending_next_block = -1;
            int32_t first_pending_logical_blocks = -1;
            unsigned first_pending_flags = 0;
            for_each_fanin_slot_state(*payload, [&](PTO2TaskSlotState *producer) {
                PTO2TaskState producer_state = producer->task_state.load(std::memory_order_acquire);
                if (producer_state == PTO2_TASK_PENDING) {
                    producer_pending++;
                    if (first_pending_producer == -1) {
                        PTO2TaskDescriptor *producer_task = producer->task;
                        first_pending_producer =
                            producer_task != nullptr ? static_cast<int64_t>(producer_task->task_id.raw) : -2;
                        first_pending_fanin_refcount = producer->fanin_refcount.load(std::memory_order_acquire);
                        first_pending_fanin_count = producer->fanin_count;
                        first_pending_completed_subtasks = producer->completed_subtasks.load(std::memory_order_acquire);
                        first_pending_total_subtasks = producer->total_required_subtasks;
                        first_pending_next_block = producer->next_block_idx.load(std::memory_order_acquire);
                        first_pending_logical_blocks = producer->logical_block_num;
                        first_pending_flags = producer->lifecycle_flags.load(std::memory_order_acquire);
                    }
                } else if (producer_state == PTO2_TASK_COMPLETED) {
                    producer_completed++;
                } else if (producer_state == PTO2_TASK_CONSUMED) {
                    producer_consumed++;
                }
            });
            if (producer_pending > 0) {
                LOG_ERROR(
                    ...
                    first_pending_fanin_count, first_pending_completed_subtasks, first_pending_total_subtasks,
            ...
            LOG_ERROR(
                ...
                producer_pending, producer_completed, producer_consumed,
                ...
                static_cast<unsigned>(payload->early_dispatch_state.load(std::memory_order_acquire)),
                static_cast<unsigned>(payload->early_sync_drain_state.load(std::memory_order_acquire))
            );
        }
    }
    ...
    LOG_ERROR(
        ...
        "heap_waiting=1",
        static_cast<unsigned>(head_ring_id), head_ring.fc.current_task_index.load(std::memory_order_acquire),
        head_task_id, static_cast<int32_t>(head_state), outstanding, matching, pending, pending_ready, completed,
        consumed, (fanout_refcount & PTO2_FANOUT_SCOPE_BIT) != 0, logged_pending_consumers, head_still_current
    );
    return head_still_current;
}
#endif
...
static bool prepare_task(
    ...
    out->alloc_result = allocator.alloc(
        total_output_size, orch->sm_header, orch->scheduler_runs_concurrently, orch->scheduler, log_heap_hol_consumers
    );
#else
