 ...
 * which init entry runs first. simpler_init claims program mode — the
 * historical exclusive-device path driven through the prepared-run family
 * above. simpler_kernel_mode_init claims kernel mode, which borrows the
 ...
 * capturable by ACLGraph as an ordinary node. The claim is the single mode
 * source (ExecutionModeClaimState on the platform runner): every kernel-mode
 * guard on the device/ACL lifecycle and arena paths keys on it, so an init
 * that claims the mode arms the guards as part of accepting the context,
 * with no separate declaration call to forget. Claiming is the init
 * implementation's obligation, and no implementation of the entries below
 * claims, so a context's mode stays unclaimed and the guards never fire.
 *
 ...
 * resources and never reset the device or finalize ACL. Onboard, the
 * platform finalize() returns early while device_id_ is unset, and only the
 * device-owning program path sets device_id_, so it releases nothing for a
 * kernel context and its kernel-mode branch is unreachable.
 *
 ...
 *
 * A successful call claims kernel mode for the context — mutually exclusive
