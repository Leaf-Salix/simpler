...
int32_t execute_kernel_round_impl(
    ...
    if (arrival == RoundArrival::Finalizer) {
        ...
        if (!gate.publish_final_status(ticket, sm_status, cleanup) || !gate.read_final_status(ticket, &result))
            return -1;
        if (executor.kernel_control_attached_)
            executor.kernel_cores_.publish_status(result.runtime_status, result.cleanup_status);
