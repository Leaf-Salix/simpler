...
public:
    bool join(int32_t launched, int32_t reported_cpu, KernelRoundTicket *out) noexcept {
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                if (!claims_.compare_exchange_weak(claim, stamp(epoch, index + 1), std::memory_order_acq_rel))
                    continue;
    ...
    bool publish_admission(
        ...
        for (int32_t i = 0; i < launched; ++i) {
            ...
            while (role < count && filled[role]) ++role;
            if (role == count) break;
            slots_[i].execution_index = role;
            filled[role++] = true;
        }
    ...
    bool publish_init_verdict(const KernelRoundTicket &leader) noexcept {
        return publish_init_verdict(leader, []() noexcept { return 0; });
    }
    ...
    bool publish_final_status(
        const KernelRoundTicket &finalizer, int32_t sm_status, int32_t cleanup_status
    ) noexcept {
...
private:
    enum class Stage : uint8_t {
        Idle, Starting, Joining, Admitting, Admitted, Initializing, Initialized, Final,
        Vacant, Joined, ReadingAdmission, AdmissionRead, InitWriting, InitDone, InitPublishing, ReadingInit, ReadyToRun,
        Arriving, Arrived, Finalizer, Finalizing, ReadingFinal, FinalRead, Departed, Retiring, Retired
    };
