...
inline bool validate_tmr_kernel_clear_plan(
    const TmrKernelClearPlan &plan, const TmrKernelClearBinding &trusted_binding
) noexcept {
    ...
    for (size_t i = 0; i < expected.regions.size(); ++i) {
        if (plan.regions[i].address != expected.regions[i].address || plan.regions[i].bytes != expected.regions[i].bytes)
            return false;
    }
