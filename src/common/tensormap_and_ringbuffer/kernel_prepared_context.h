...
inline bool disjoint(Region a, Region b) noexcept {
    return a.base + a.bytes <= b.base || b.base + b.bytes <= a.base;
}
...
inline bool make_prepared_kernel_context(
    ...
    if (descriptor.execution_threads < 2 || descriptor.execution_threads > PLATFORM_MAX_AICPU_THREADS ||
        ...
        config.aicpu_thread_num != descriptor.execution_threads ||
        config.aicpu_allowed_cpu_count != descriptor.execution_threads || config.worker_count != descriptor.worker_count ||
        config.ready_queue_shards <= 0 || config.ready_queue_shards > PLATFORM_MAX_AICPU_THREADS)
        return false;
    ...
    // Kernel prepare publishes the static GM/SM and prebuilt-arena addresses
    // before this registration snapshot is taken.  They are resident inputs,
    // not per-invocation state; the dispatch path still uses the explicit
    // binding view below.  Only callable/task fields must remain empty here.
    if (config.active_callable_id_ != -1 || config.orch_args_storage_.tensor_count() != 0 ||
