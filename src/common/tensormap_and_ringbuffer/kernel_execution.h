...
inline int32_t invocation_dispatch_status(InvocationStatus status) noexcept {
    switch (status) {
    case InvocationStatus::Ok: return 0;
    case InvocationStatus::StaleCallable: return static_cast<int32_t>(KernelDispatchStatus::Stale);
    case InvocationStatus::InvalidBinding: return static_cast<int32_t>(KernelDispatchStatus::InvalidBinding);
    default: return static_cast<int32_t>(KernelDispatchStatus::InvalidArgs);
    }
}
...
int32_t execute_kernel_round(
    const KernelExecutionRequest &, int32_t reported_cpu, KernelFinalStatus *out = nullptr
) noexcept;
