...
inline bool make_prepared_kernel_callable(const TmrCallableRegistrationArgs &resident, PreparedKernelCallable *out) {
    if (out == nullptr || resident.context_generation == 0 || resident.callable_id < 0 ||
        resident.callable_id >= MAX_REGISTERED_CALLABLE_IDS || resident.reserved != 0 || resident.bytes > SIZE_MAX ||
    ...
    candidate.device_address = resident.device_address;
    candidate.bytes = resident.bytes;
    candidate.identity.callable_id = resident.callable_id;
    if (simpler::kernel::derive_invocation_counts(
            image->signature_, image->sig_count(), &candidate.identity.tensor_count, &candidate.identity.scalar_count
        ) != InvocationStatus::Ok)
        return false;
    size_t count = 0;
