...
int register_kernel_context(Executor &executor, const void *arg) noexcept {
    ...
    if (descriptor.resident_runtime == 0 || descriptor.resident_runtime % alignof(Runtime) != 0 ||
        descriptor.resident_runtime > UINTPTR_MAX - sizeof(DeviceRuntimeLaunchDesc) || descriptor.resident_kernel_args == 0 ||
        descriptor.resident_kernel_args % alignof(KernelArgs) != 0 ||
    ...
    if (args.runtime_args != resident || args.regs == 0 || args.regs % alignof(uint64_t) != 0 ||
        ...
        LOG_ERROR("simpler_aicpu_prepare_tmr_context: KernelArgs validation failed");
        return -1;
    }
    PreparedKernelContext candidate;
    if (!make_prepared_kernel_context(registration, descriptor, *resident, &candidate)) {
        LOG_ERROR("simpler_aicpu_prepare_tmr_context: descriptor validation failed");
        return -1;
    }
    if (executor.kernel_context_ready_) {
        ...
        return std::memcmp(&old.descriptor, &candidate.descriptor, sizeof(descriptor)) == 0 &&
                       old.residency_base == candidate.residency_base && old.residency_count == candidate.residency_count &&
                       old.stride == candidate.stride && old.allowed_cpus == candidate.allowed_cpus
                   ? 0 : -1;
    }
...
int register_kernel_callable(Executor &executor, const void *arg) noexcept {
    if (arg == nullptr || !executor.kernel_context_ready_ || !executor.kernel_gate_.idle() ||
        executor.kernel_invocation_.active()) return -1;
    ...
    if (registration.context_generation != context.descriptor.context_generation || registration.reserved != 0 ||
        ...
        registration.slot_generation == 0 || !context.contains_residency(registration.residency_address)) return -1;
    ...
    if (resident.callable_id != registration.callable_id || resident.generation != registration.slot_generation ||
        resident.context_generation != context.descriptor.context_generation || resident.reserved != 0 ||
        resident.bytes > SIZE_MAX || resident.device_address == 0 ||
        resident.bytes > UINTPTR_MAX - resident.device_address) return -1;
    ...
    if (slot.kernel.residency_address != 0) {
        ...
        return old.residency_address == registration.residency_address &&
                       old.residency.device_address == resident.device_address && old.residency.bytes == resident.bytes &&
                       old.residency.generation == resident.generation ? 0 : -1;
    }
    ...
    try {
        cache_invalidate_range(reinterpret_cast<const void *>(resident.device_address), static_cast<size_t>(resident.bytes));
        ...
        if (executor.load_orch_so(registration.callable_id, reinterpret_cast<uint64_t>(image->binary_data()),
                                 image->binary_size(), image->func_name(), image->config_name(), 0) != 0) return -1;
...
int release_kernel_context(Executor &executor, const void *arg) noexcept {
    ...
    if (registration.descriptor_address != context.descriptor.self_address ||
        registration.context_generation != context.descriptor.context_generation) return -1;
    ...
    for (auto &slot : executor.orch_so_table_) slot.kernel = {};
...
template <typename Executor>
int release_kernel_callable(Executor &executor, const void *arg) noexcept {
    if (arg == nullptr || !executor.kernel_context_ready_ || !executor.kernel_gate_.idle() ||
        executor.kernel_invocation_.active())
        return -1;
    TmrCallableRegistrationArgs registration{};
    std::memcpy(&registration, arg, sizeof(registration));
    if (registration.context_generation != executor.kernel_context_.descriptor.context_generation ||
        registration.callable_id < 0 || registration.callable_id >= MAX_REGISTERED_CALLABLE_IDS ||
        registration.slot_generation == 0 || !executor.kernel_context_.contains_residency(registration.residency_address))
        return -1;
    auto &slot = executor.orch_so_table_[registration.callable_id];
    if (!slot.kernel_owned || slot.kernel.residency_address != registration.residency_address ||
        slot.kernel.identity.slot_generation != registration.slot_generation)
        return -1;
    slot.kernel = {};
    slot.kernel_owned = false;
    return 0;
}
...
int dispatch_prepared_kernel_task(Executor &executor, void *arg, int32_t cpu) noexcept {
    LOG_ERROR("TMR dispatch enter cpu=%d", cpu);
    ...
    if (arg != nullptr) {
        ...
        if (dispatch.packet_bytes >= sizeof(dispatch) && dispatch.packet_bytes <= SIZE_MAX &&
            header.payload_bytes == dispatch.packet_bytes - sizeof(dispatch) &&
            header.mode == SIMPLER_MODE_KERNEL && header.generation != 0 &&
            simpler::kernel::valid_invocation_counts(header.tensor_count, header.scalar_count) &&
            header.host_copy_tensor_count == 0 &&
            header.callable_id >= 0 && header.callable_id < MAX_REGISTERED_CALLABLE_IDS) {
            ...
            if (slot.residency_address != 0 && dispatch.residency_address == slot.residency_address &&
                ...
                if (kernel_callable_residency_matches(header, resident) &&
                    resident.context_generation == context.descriptor.context_generation &&
                    resident.generation == slot.identity.slot_generation &&
                    resident.device_address == slot.residency.device_address && resident.bytes == slot.residency.bytes) {
                    // The packet's TMR binding ref names the callable
                    // residency descriptor.  `context.binding.identity` is a
                    // different identity (the resident context descriptor)
                    // used by the provider.  Do not pass the latter to the
                    // invocation decoder: doing so rejects every otherwise
                    // valid launch as InvalidBinding before the round gate.
                    request.binding.identity = {
                        dispatch.residency_address, context.descriptor.context_generation
                    };
                    constexpr size_t prefix = offsetof(SimplerKernelDispatchArgs, invocation);
                    request.packet = {static_cast<const uint8_t *>(arg) + prefix,
                                      static_cast<size_t>(dispatch.packet_bytes) - prefix};
    ...
    if (status != 0 && executor.kernel_context_.handshake.control != nullptr) {
        int32_t expected = 0;
        __atomic_compare_exchange_n(&executor.kernel_context_.handshake.control->runtime_status, &expected,
                                    static_cast<int32_t>(status), false, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
    }
    LOG_ERROR("TMR dispatch exit cpu=%d status=%d runtime=%d cleanup=%d", cpu, status, result.runtime_status,
              result.cleanup_status);
    if (result.cleanup_status != 0) return static_cast<int>(KernelDispatchStatus::CleanupFailed);
