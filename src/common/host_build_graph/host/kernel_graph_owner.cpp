...
int DeviceRunnerBase::prepare_hbg_kernel_runtime(const HostApi *) {
    ...
    return aclrtSynchronizeStreamWithTimeout(control_stream, PLATFORM_STREAM_SYNC_TIMEOUT_MS);
}
...
int DeviceRunnerBase::prepare_hbg_kernel_callable_registration(
    ...
    auto &context = *hbg_kernel_state_;
    int rc = 0;
    if (!context.slot_registered) return PTO_RUNTIME_ERR_INVALID_STATE;
