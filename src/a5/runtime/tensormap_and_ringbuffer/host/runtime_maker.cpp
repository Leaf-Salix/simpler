...
extern "C" int validate_runtime_impl(Runtime *runtime) {
    ...
    if (runtime_status != 0) {
        ...
        // Read fanin dedup profiling. sm_header works on real hardware
        // (host/AICPU share physical memory); Runtime object works on sim
        // (same process). Use whichever has non-zero data.
