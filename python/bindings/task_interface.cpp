...
nb::capsule make_kernel_queue_handle(std::shared_ptr<simpler::kernel::KernelHostOwner> owner);
...
NB_MODULE(_task_interface, m) {
    ...
    nb::class_<ChipWorker>(m, "_ChipWorker")
        ...
        .def(
            ...
            [](ChipWorker &self, int32_t callable_id, const PyChipCallable &callable) {
                self.kernel_prepare_callable(callable_id, callable.buffer_.data(), callable.buffer_.size());
            },
            nb::arg("callable_id"), nb::arg("callable"), nb::call_guard<nb::gil_scoped_release>(),
            "Synchronously prepare a callable on the context's dedicated stream, outside capture."
        )
        .def(
            "kernel_launch",
        ...
        .def(
            "_queue_handle",
            [](ChipWorker &self) {
                return make_kernel_queue_handle(self.kernel_owner());
            }
        )
        .def_prop_ro("_has_kernel_context", &ChipWorker::has_kernel_context)
        .def_static(
    ...
    // These helpers own only caller-side streams. Kernel context streams are
    // separate resources; the Torch adapter uses the framework's current stream.
    m.def(
        "_acl_create_stream",
        []() {
            acl_api().init();
