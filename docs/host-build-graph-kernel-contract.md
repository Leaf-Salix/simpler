...
Context init also registers the sealed slot on the dedicated AICPU stream and
waits for it before returning; init must run outside capture. Later Host builds
...
prepare/freeze and slot registration. Callable prepare uploads context-owned
...
First-prepare and second-prepare cases cover global and thread-local capture,
reject any blocking synchronization through an interposer, and execute the new
