...
TEST(TmrKernelExecutionRoundTest, SlowFinalizerKeepsStorageUntilAllDeparturesAndRejectsOverwrite) {
    ExecutorModel executor;
    Signal publishing;
    ...
    std::condition_variable returned_condition;
    int32_t returned = 0;
    std::vector<std::thread> threads;
    for (size_t i = 0; i < results.size(); ++i) {
        threads.emplace_back([&, i] {
            results[i] = execute_kernel_round_impl(executor, request, static_cast<int32_t>(10 + i));
            std::lock_guard<std::mutex> lock(returned_mutex);
            ++returned;
    ...
    publishing.wait();
    {
        std::unique_lock<std::mutex> lock(returned_mutex);
        returned_condition.wait(lock, [&] {
            return returned == ExecutorModel::kLaunchedThreads - 1;
        });
    }
    EXPECT_EQ(executor.clears.load(), 0);
    EXPECT_TRUE(executor.kernel_invocation_.active.load());
